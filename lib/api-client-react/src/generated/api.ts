...
// ── Manually added hooks for new report endpoints ─────────────────────────────

import type { ClinicStat, ProgramDetails } from './api.schemas';

// GET /api/reports/clinic-stats
export const getClinicStatsUrl = () => `/api/reports/clinic-stats`;

export const getClinicStats = async (options?: RequestInit): Promise<ClinicStat[]> => {
  return customFetch<ClinicStat[]>(getClinicStatsUrl(), { ...options, method: 'GET' });
};

export const getGetClinicStatsQueryKey = () => [`/api/reports/clinic-stats`] as const;
...
export function useGetClinicStats<TData = Awaited<ReturnType<typeof getClinicStats>>, TError = ErrorType<unknown>>(
  ...
  const queryFn = ({ signal }: { signal?: AbortSignal }) => getClinicStats({ signal, ...options?.request });
  return useQuery({ queryKey, queryFn, staleTime: 5 * 60 * 1000, ...queryOptions }) as UseQueryResult<TData, TError> & { queryKey: typeof queryKey };
}

// GET /api/reports/program-details/:programId
export const getProgramDetailsUrl = (programId: string) => `/api/reports/program-details/${programId}`;

export const getProgramDetails = async (programId: string, options?: RequestInit): Promise<ProgramDetails> => {
  return customFetch<ProgramDetails>(getProgramDetailsUrl(programId), { ...options, method: 'GET' });
};

export const getGetProgramDetailsQueryKey = (programId: string) => [`/api/reports/program-details/${programId}`] as const;

export function useGetProgramDetails<TData = Awaited<ReturnType<typeof getProgramDetails>>, TError = ErrorType<unknown>>(
  programId: string,
  options?: { query?: UseQueryOptions<Awaited<ReturnType<typeof getProgramDetails>>, TError, TData>; request?: SecondParameter<typeof customFetch> }
) {
  const { query: queryOptions } = options ?? {};
  const queryKey = queryOptions?.queryKey ?? getGetProgramDetailsQueryKey(programId);
  const queryFn = ({ signal }: { signal?: AbortSignal }) => getProgramDetails(programId, { signal, ...options?.request });
  return useQuery({ queryKey, queryFn, enabled: !!programId, staleTime: 5 * 60 * 1000, ...queryOptions }) as UseQueryResult<TData, TError> & { queryKey: typeof queryKey };
}
