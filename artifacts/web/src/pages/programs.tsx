  ...
  return (
    <div className="page-container animate-in-up">
      <PageHeader
        ...
        description={`${data?.meta?.total ? `${data.meta.total} ` : ""}Care pathways and treatment programs.`}
        actions={
          <>
      ...
      <div className="p-8 space-y-4">
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-24 text-muted-foreground glass-card rounded-3xl border border-border/50">
            <div className="w-20 h-20 bg-muted/30 rounded-full flex items-center justify-center mb-6">
              <FolderGit2 className="w-10 h-10 opacity-40 text-primary" />
            </div>
            <h3 className="font-semibold text-xl text-foreground">No clinical programs</h3>
            <p className="text-sm mt-2 max-w-sm text-center">
              {search ? "No programs match your search query." : "You have not created any clinical programs yet."}
            </p>
            {!search && (
              <Button className="mt-8 bg-primary hover:bg-primary/90 text-primary-foreground rounded-xl px-6" onClick={openCreate}>
            ...
            {filteredPrograms.map((program, index) => {
              ...
              return (
                <Card
                  ...
                  className="group hover:shadow-lg hover:-translate-y-1 transition-all duration-300 glass-card rounded-2xl border border-border/60 hover:border-primary/30 animate-in-up flex flex-col"
                  style={{ animationDelay: `${index * 40}ms` }}
                >
                  <CardContent className="p-6 flex-1">
                    <div className="flex items-start justify-between mb-4">
                      <div className={`w-12 h-12 rounded-xl flex items-center justify-center group-hover:scale-110 transition-transform duration-300 border border-primary/10 ${colorClass.split(" ").slice(0, 2).join(" ")}`}>
                        <FolderGit2 className={`w-6 h-6 ${colorClass.split(" ").slice(2).join(" ")}`} />
                      </div>
                      <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        <Button size="icon" variant="ghost" className="h-8 w-8 bg-background border border-border shadow-sm hover:bg-primary hover:text-primary-foreground" onClick={() => openEdit(program)}>
                    ...
                    <h3 className="font-bold text-lg leading-tight text-foreground group-hover:text-primary transition-colors">{program.name}</h3>
                  ...
                  <CardFooter className="px-6 pb-6 pt-0 mt-auto">
                    <div className="flex items-center justify-between w-full pt-4 border-t border-border/50">
                      ...
                      {(program as any).enrollmentCount !== undefined && (
                        <Badge variant="secondary" className="text-xs font-bold bg-muted/50 border-border/50">
